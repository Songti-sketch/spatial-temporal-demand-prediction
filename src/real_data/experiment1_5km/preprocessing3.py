...
class Preprocessor:
    ...
    def compute_fixed_assortments(df, dist, num_total_cells, dist_threshold=5.0):
        # fixed_assortments = {}
        # for grid in range(num_total_cells):
        #     grid_df = df[df['cust_index'] == grid]
        #     chosen_sellers = grid_df['sell_index'].unique()
        #     seller_distances = dist[grid, chosen_sellers]
        #     nearby_sellers = chosen_sellers[seller_distances <= dist_threshold]
        #     fixed_assortments[grid] = nearby_sellers
